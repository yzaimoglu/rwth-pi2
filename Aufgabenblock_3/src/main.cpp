...
void vAufgabe_8() {
	try {
		std::string fileName = "VO.dat";
		std::ifstream file;
		file.open(fileName);

		if (!file) throw std::runtime_error(fileName + " kann nicht geöffnet werden");

		Kreuzung kreuzung;
		PKW pkw;
		Fahrrad fahrrad;
		file >> pkw >> fahrrad >> kreuzung;
		file.close();

		std::cout << "Erstellte Simulationsobjekte: " << std::endl << std::endl;
		Fahrzeug::vKopf();
		std::cout << pkw << std::endl;
		std::cout << fahrrad << std::endl;
		std::cout << kreuzung << std::endl << std::endl;
...
void vAufgabe_9() {
	std::string sDateiName = "Simu.dat";
	std::ifstream fDatei(sDateiName);

	if (!fDatei) {
		std::cerr << "Fehler beim Öffnen der Datei " << sDateiName << std::endl;
		exit(1);
	}

	Simulation simulationVersuch;

	try {
		simulationVersuch.vEinlesen(fDatei);
	} catch (std::runtime_error & error) {
		std::cerr << "RUNTIME_ERROR: " << std::endl << error.what() << std::endl;
		exit(1);
	}

	std::cout << "Anfangen der Simulation!" << std::endl;

	simulationVersuch.vSimulieren(5, 0.2);
}

void vAufgabe_9a() {
	std::ifstream fDatei("SimuDisplay.dat");

	if (!fDatei) {
		std::cerr << "Die Datei SimuDisplay.dat konnte nicht geoeffnet werden" << std::endl;
		exit(1);
	}
	...
	try {
		simulationVersuch.vEinlesen(fDatei, true);
	} catch (std::runtime_error &error) {
		std::cerr << "RUNTIME_ERROR: " << std::endl << error.what() << std::endl;
		exit(1);
	}

	std::cout << "Anfangen der Simulation!" << std::endl << std::endl;

	simulationVersuch.vSimulieren(10, 0.1);
}
